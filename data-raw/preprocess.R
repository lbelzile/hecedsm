...
LRMM23_S1 <- haven::read_sav(file = "data-raw/source/LRMM22_S1.sav") |>
  dplyr::select(DV, self0, form, gender, age) |>
  dplyr::rename(appreciation = DV, role = self0) |>
  dplyr::mutate(
    role = factor(role, levels = c(0, 1), labels = c("initiator", "responder")),
...
JCD14_S2 <- haven::read_sav(
  ...
  mutate(
    Condition = factor(Condition, labels = c("control", "cleanliness")),
    Age = as.integer(Age),
...
GK14_S3 <- gk_s3_data_raw |>
  ...
  select(age.group, Condition, RES_1:RES_4) |>
  ...
  mutate(age = haven::as_factor(age), condition = haven::as_factor(condition))
...
BSJ92 <-
  ...
  mutate(group = fct_recode(.f = group, DR = "Basal", TA = "Strat")) |>
  tibble::as_tibble()
...
LBJ17_S1A <- lages |>
  pivot_longer(
    cols = NA10P1:WA25P5, # all columns in this range
  ...
  transmute(
    nerror = nerror,
    #normalacuity = NA.,
    adaptation = factor(adaptation, labels = c("PA", "AC")),
    ...
    id = factor(ObsID),
    acuity = factor(Group, labels = c("normal", "high")),
  )
LBJ17_S1A |>
  ...
  ggplot(
    mapping = aes(y = mean, x = fontsize, col = adaptation, pch = acuity)
  ) +
  geom_point()
...
MULTI21_D1 <- FBC |>
  dplyr::rename(lab = lablocale, age = age_group) |>
  ...
  tidyr::pivot_longer(
    cols = c("never", "sometimes", "usually"),
    names_to = "frequency",
    values_to = "count"
  ) |>
  tibble::as_tibble() |>
...
MULTI21_D2 <- read.csv(
  "data-raw/source/Multilab2021_spanData_allLabs.csv",
  header = TRUE
) |>
  ...
  mutate(
    ...
    frequency = factor(frequency, labels = tolower(sort(unique(frequency))))
  ) |>
  dplyr::arrange(lab, id) |>
  tibble::as_tibble() |>
  dplyr::mutate(
    age = factor(age, levels = levels(age)[order(c(10, 5, 6, 7))])
  ) |>
  dplyr::mutate(
    timing = factor(
      timing,
      labels = c("delayed", "immediate", "point and name")
    )
  ) |>
  mutate(
    order = ifelse(
      (taskorder == "first" & timing == "immediate") |
        (taskorder == "second" & timing == "delayed"),
      0L,
      1L
    )
  ) |>
  ...
  mutate(
    taskorder = factor(
      taskorder,
      levels = c(0L, 1L),
      labels = c("immediate first", "delayed first")
    )
  ) |>
  select(!order)
...
RJ74 <- data.frame(
  gender = factor(rep(c("male", "female"), length.out = 8L)),
  decision = factor(rep(
    rep(c("promote", "hold file"), each = 2L),
    length.out = 8L
  )),
...
MP14_S1 <- read_csv(
  "https://edsm.rbind.io/data/MaglioPolman2014S1.csv",
  col_types = "fiff"
) |>
  # STN_NUMBER redundant with STN_NAME
  transmute(
    station = fct_relevel(factor(STN_NAME, labels = labs), olabs),
...
BMH19_E2acc <- BMH19_E2raw |>
  select(pnum:diffmon) |>
  pivot_longer(
    cols = starts_with(c("diff")),
    names_to = "color",
    names_prefix = c("diff"),
    values_to = c("accuracy")
  )
BMH19_E2pcor <- BMH19_E2raw |>
  select(pnum, samecol:samemon) |>
  pivot_longer(
    cols = starts_with(c("same")),
    names_to = "color",
    names_prefix = c("same"),
    values_to = c("pcorr")
  )
...
C22 <- readxl::read_xlsx("data-raw/source/Verdictspottingdata.xlsx") |>
  ...
  mutate(
    id = factor(participant), # cast explanatories to factor
    vorder = factor(vignette_order),
    anchor = factor(
      anchor,
      levels = 1:2,
      labels = c("weak-first", "strong-first")
    ),
...
SSVB21_S2 <- read.csv(
  "data-raw/source/vanStekelenburg2021S2.csv",
  header = TRUE
) |>
  tibble::as_tibble() |>
  transmute(prior = prior, post = post, condition = factor(condition))
usethis::use_data(SSVB21_S2, overwrite = TRUE)


SSVB21_S3 <- read.csv(
  "data-raw/source/vanStekelenburg2021S3.csv",
  header = TRUE
) |>
  filter(Prior > 0) |>
  mutate(
    Condition = factor(
      Condition,
      labels = c("BoostPlus", "consensus", "control")
    )
  ) |>
  tibble::as_tibble() |>
  transmute(
...
RH19_S1 <- haven::read_sav("data-raw/source/Roczniewska_Higgins_2019.sav") |>
  transmute(
    change = factor(change_outcome, labels = c("negative", "positive")),
    regfocus = factor(
      reg_focus_manipulation,
...
LC19_S1 <- read.csv("data-raw/source/LeeChoi2019S1.csv") |>
  ...
  mutate(
    consistency = factor(cond, labels = c("consistent", "inconsistent")),
...
LC19_S2 <- read.csv("data-raw/source/LeeChoi2019S2.csv", sep = "\t") |>
  select(!cond) |>
  mutate(
    consistency = factor(image, labels = c("inconsistent", "consistent")),
    Gender = factor(Gender, labels = c("male", "female")),
    prodeval = (att1 + att2 + att3) / 3,
    fluency = ifelse(
      is.na(info2),
      info3,
      ifelse(is.na(info3), info2, (info2 + info3) / 2)
    )
  ) |>
...
LRMM23_S3 <- haven::read_sav(file = "data-raw/source/LRMM22_S3.sav") |>
  ...
  mutate(
    apprec_resp = as.integer(apprec_resp),
...
LRMM23_S5b <- haven::read_sav("data-raw/source/LRMM22_S5b.sav") |>
  transmute(
    ...
    condition = factor(condition, labels = c("other", "self")),
    age = as.integer(age),
    gender = as_factor(gender),
  ) |>
  dplyr::mutate(
    initiator = factor(ifelse(condition == "self", "initiator", "responder"))
  ) |>
...
BL22_L <- rbind(d1, d2) |>
  tibble::as_tibble() |>
  mutate(cond = factor(cond), study = factor(study)) |>
  rename(
    nidea = idea_number,
...
# Eyetracker data
BL22_E <- read.csv(
  "data-raw/source/Brucks_Levav_2022/eyegaze_data_by_participant.csv"
) |>
  tibble::as_tibble() |>
  rename(id = Group.Number, ) |>
  mutate(
    id = factor(as.integer(factor(id))),
    writer = writer == "w",
    cond = factor(cond)
  ) |>
  select(!total_time)
...
levels(DA23_E1$format) <- c("quantity-integrated", "quantity-sequential")
...
DA23_E2 <- read.csv("data-raw/source/DukeAmir2022E2.csv") |>
  ...
  dplyr::rename(meanval = mean_val, elength = explanation_length) |>
  tibble::as_tibble()
levels(DA23_E2$format) <- c("quantity-integrated", "quantity-sequential")
...
arithmetic <- arithmetic |>
  dplyr::mutate(
    group = factor(
      group,
      labels = c("control 1", "control 2", "praise", "reprove", "ignore")
    )
  ) |>
  dplyr::rename(score = y)
usethis::use_data(arithmetic, overwrite = TRUE)

popcorn <- read.table("data-raw/source/popcorn.txt", header = TRUE) |>
  tibble::as_tibble() |>
  dplyr::mutate(
    brand = factor(brand, labels = c("national1", "national2", "local")),
    power = factor(power, labels = c("500W", "600W")),
    time = factor(time, labels = c("4m", "4m30s", "5m"))
  )
usethis::use_data(popcorn, overwrite = TRUE)

teller <- read.table("data-raw/source/teller.txt", header = TRUE) |>
  tibble::as_tibble() |>
  dplyr::select(!id) |>
  dplyr::mutate(course = factor(tolower(course)), nweeks = factor(nweeks))
usethis::use_data(teller, overwrite = TRUE)

words <- read.table("data-raw/source/words.txt", header = TRUE) |>
  tibble::as_tibble() |>
  mutate(
    feedback = factor(feedback, labels = c("none", "positive", "negative")),
    material = factor(
      material,
      labels = c(
        "low freq/low emotion",
...
data <- readxl::read_xlsx(
  'data-raw/source/Sharma_Tully_Cryder-2021-Sup_study5.xlsx'
)
...
STC21_SS5 <- data |>
  ...
  dplyr::transmute(
    purchase = factor(tolower(Cond_Purchase)),
...
WUTEF22_S1 <- data |>
  ...
  transmute(
    ...
    fin_literacy = factor(as.integer(
      I(q125 == 1) + I(q127 == 5) + I(q128 == 4)
    )),
...
HB22_S5 <- haven::read_sav("data-raw/source/JCR 2022 Study 5.sav") |>
  ...
  rename(curstate = condition_state_now, predout = condition_dv_peace_war) |>
  mutate(
    likelihood_1yr = case_when(
  ...
  select(c(age, gender, curstate, predout, likelihood_1yr, likelihood_20yr)) |>
  mutate(
    id = 1:n(),
    age = as.integer(age) + 17L,
    gender = factor(gender, labels = tolower(levels(as_factor(gender)))),
...
sexe <- readxl::read_xlsx(
  path = "data-raw/source/data_Tech3Lab.xlsx",
  sheet = 3
)$Sexe
LJLSFBM20 <- readxl::read_xlsx(
  path = "data-raw/source/data_Tech3Lab.xlsx",
  sheet = 4
) |>
  rename_all(tolower) |>
  mutate(
    subject_id = factor(as.integer(relevel(factor(subject_id), ref = "35"))),
...
BRLS21_T3 <- readxl::read_xlsx(
  path = "data-raw/source/data_Tech3Lab.xlsx",
  sheet = 2
) |>
  rename_all(tolower) |>
  mutate(
    task = factor(task, labels = c("phone", "watch", "speaker", "texting")),
...
BRLS21_EDA <- read.csv(
  ...
  tibble::as_tibble() |>
  mutate(
    relative_time = as.integer(relative_time / 1000L),
    id = factor(as.integer(factor(participant))),
    task = factor(
      tache,
      labels = c("baseline", "cell", "voicecall", "watch", paste0("task", 1:6))
    )
  ) |>
  filter(!task %in% paste0("task", 1:6)) |>
  select(!c(absolute_time, tache, participant)) |>
...
data_exp3$Predicted_Intrinsic_Motivation_Index_LONG <- as.numeric(as.character(
  data_exp3$Predicted_Intrinsic_Motivation_Index_LONG
))
data_exp3$Predicted_Intrinsic_Motivation_Index_SHORT <- as.numeric(as.character(
  data_exp3$Predicted_Intrinsic_Motivation_Index_SHORT
))
data_exp3$Experienced_Intrinsic_Motivation_Index <- as.numeric(as.character(
  data_exp3$Experienced_Intrinsic_Motivation_Index
))

data_exp3_melt <- melt(
  data_exp3,
  id.vars = "Waiting_Condition",
  measure.vars = c(
    "Predicted_Intrinsic_Motivation_Index_LONG",
    "Predicted_Intrinsic_Motivation_Index_SHORT",
...
HOSM22_E3 <- subset(
  data_exp3_melt,
  (!(data_exp3_melt$Waiting_Condition == "L" &
...
HOSM22_E3$variable <- ifelse(
  HOSM22_E3$variable == "Predicted_Intrinsic_Motivation_Index_LONG" |
    HOSM22_E3$variable == "Predicted_Intrinsic_Motivation_Index_SHORT",
  "prediction",
  "experience"
)
colnames(HOSM22_E3) <- c(
  "Waiting_Condition",
  "Rating_type",
...
HOSM22_E3$Waiting_Condition <- ifelse(
  HOSM22_E3$Waiting_Condition == "L",
  "long",
  "short"
)
...
data_exp4_subset <-
  melt(
    data_exp4,
    id.vars = c("Participant_ID", "Assigned_Number"),
    measure.vars = c(
      "Predicted_Intrinsic_Motivation_Index_INTERIM",
...
data_exp4_subset$intrinsic_motivation_score <- as.numeric(as.character(
  data_exp4_subset$intrinsic_motivation_score
))

data_exp4_subset$rating_type <- ifelse(
  data_exp4_subset$rating_type ==
    "Experienced_Intrinsic_Motivation_Index_INTERIM",
  1,
  -1
)
...
data_exp4_subset$centered_interim_rating_time <- as.numeric(as.character(
  data_exp4_subset$interim_rating_time
)) -
  10
...
HOSM22_E4 <- data_exp4_subset |>
  tibble::as_tibble() |>
  transmute(
    id = factor(as.numeric(factor(Participant_ID))),
...
s1 <- read.csv(
  "data-raw/source/ResearchBox54/Data/Study 1 - Choose Charity.csv"
)

s1$donate01 <- ifelse(
  s1$Condition == "Open-ended" &
    s1$openended == 0 |
...
MV23_S1 <- s1d |>
  dplyr::select(donatebefore, donate01, Condition, amount) |>
  dplyr::rename(
    before = donatebefore,
    donate = donate01,
    condition = Condition
...
s4$amount0 <- ifelse(
  ...
  ifelse(
    ...
    s4$openended_1_TEXT,
    ifelse(
      s4$Condition == "Quantity1" & s4$quantity1 == 888,
      s4$quantity1_TEXT,
      ifelse(
        s4$Condition == "Quantity1" & s4$quantity1 > 0,
        s4$quantity1,
        ifelse(
          s4$Condition == "Quantity5" & s4$quantity5 == 888,
          s4$quantity5_TEXT,
          ifelse(
            s4$Condition == "Quantity5" & s4$quantity5 > 0,
            s4$quantity5,
            ifelse(
              s4$Condition == "Quantity10" & s4$quantity10 == 888,
              s4$quantity10_TEXT,
              ifelse(
                s4$Condition == "Quantity10" & s4$quantity10 > 0,
                s4$quantity10,
...
MV23_S4 <- s4u |>
  dplyr::select(donate01, Condition, amount) |>
  dplyr::rename(donate = donate01, condition = Condition) |>
  dplyr::mutate(
    condition = factor(tolower(condition)),
    donate = as.integer(donate)
  ) |>
...
supph <- read.csv(
  'data-raw/source/ResearchBox54/Data/Supp H - Prosocial Actions.csv'
)

supph$donate01 <- ifelse(
  supph$Condition == "Open-ended" &
    supph$openended == 0 |
...
#
JZBJG22_E2 <- read.csv(
  "data-raw/source/ResearchBox511/Data/e2_data copy.csv"
) |>
  dplyr::filter(
    Finished == 1, #first narrowing down to people who finished the survey
    situationCorrect == 4,
    pilot == 0,
    flownPlane == 0,
    landedPlane == 0,
  ...
  dplyr::mutate(
    condition = factor(condition, levels = c("video", "no video")),
...
FBC66_T1 <- tibble::tibble(
  count = as.numeric(FBC66_T1_mat),
...
SKD23_S2A <- rb712rawDate |>
  dplyr::select(PEF, MCHECK_PROPORTION, proportion) |>
...
LKUK24_S3 <- haven::read_sav("data-raw/source/ResearchBox1401/Study3.sav") |>
  dplyr::mutate(
    appropriation = as.numeric(appropriation),
    ...
    gender = factor(
      Gender,
      levels = 1:4,
      labels = c("male", "female", "other/neither/both", "prefer not to answer")
    ),
    .keep = "none"
  )
...
LKUK24_S4 <- haven::read_sav("data-raw/source/ResearchBox1401/Study4.sav") |>
  dplyr::mutate(
    appropriation = as.numeric(Appropri),
    ...
    brandaction = factor(
      condit,
      levels = 1:3,
      labels = c("peeking", "permission", "control")
    ),
    age = as.integer(Age),
    gender = factor(
      Gender,
...
L22_E4 <- haven::read_spss("source/ResearchBox84/Experiment 4.sav") |>
  dplyr::rename(
    social = IV_social1socialint2alone3,
    enjoyother = enjoyment_other,
    cutfreq = dv_cutfreq,
    cutintensity = dv_cutintense
  ) |>
  dplyr::mutate(
    social = relevel(as_factor(social), ref = "alone"),
    gender = as_factor(gender),
    age = as.integer(age),
...
# Dataset 45
BM04_T2 <- data.frame(
  black = factor(rep(c("0", "1", "2"), length.out = 9)),
...
MH18_T1 <- tibble::tibble(
  ...
  period = factor(rep(
    c("3-4 weeks", "1-2 months", "2-6 months"),
    length.out = 12
  )),
...
HBSC24_S4 <- read.csv("source/ResearchBox2945/study4_data.csv") |>
  ...
  dplyr::rename(authenticity = auth_mean, words = info_wordcount)
usethis::use_data(HBSC24_S4, overwrite = TRUE)

DA23_E3 <- read.csv("source/ResearchBox602/Data/DA2022_E3.csv") |>
  dplyr::select(format, purchased, amount, first_brand, gender, age) |>
  mutate(
    gender = factor(
      gender,
      levels = 1:3,
    ...
    first_brand = factor(first_brand),
    format = factor(
      format,
      levels = c("qi", "qs"),
      labels = c("quantity-integrated", "quantity-sequential")
    ),
...
for (file in list.files("../data", full.names = TRUE)) {
  load(file)
  cat(
    sinew::makeOxygen(tools::file_path_sans_ext(basename(file))),
    file = "documentation.R",
    append = TRUE
  )
}
